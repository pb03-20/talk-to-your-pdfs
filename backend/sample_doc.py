"""
Sample architecture whitepaper document for demo and testing.
"""
...
Real-time conversational voice interaction relies on the Gemini Live API (gemini-2.5-flash).
...
3. Turn Detection: Voice activity detection (VAD) automatically truncates pending generation upon user interruption.

Section 6: Cloudflare Tunnel & Zero-Cost Self-Hosting
By pairing a local FastAPI or Node server with Cloudflare Quick Tunnels, users can deploy production-grade RAG workspaces from their own hardware without any server subscription fees, credit card requirements, or cloud function upload limits."""
        }
    ]
}
